...
class ArrayTypesSpec : DatabaseTestHelper() {
  ...
  fun `"connection" should "correctly parse the array type"`() {
    ...
    withHandler { handler ->
      try {
        ...
        assertThat(result[0]("text_column")).isEqualTo(listOf("some,\"comma,separated,text", "another line of text", "fake,backslash", "real\\,backslash\\", null))
        assertThat(result[0]("timestamp_column")).isEqualTo(listOf(
  ...
  fun `"connection" should "correctly send arrays using prepared statements"`() {
    ...
    val texts = listOf("some,\"comma,separated,text", "another line of text", "fake,backslash", "real\\,backslash\\", null)
