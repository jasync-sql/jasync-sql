...
class MySQLColumnDecoderRegistry extends ColumnDecoderRegistry {
  ...
  def decoderFor( kind : Int ) : ColumnDecoder = {
    (kind : @switch) match {
      ...
      case ColumnTypes.FIELD_TYPE_LONG => LongEncoderDecoder
      case ColumnTypes.FIELD_TYPE_LONGLONG => BigIntegerEncoderDecoder
      ...
      case ColumnTypes.FIELD_TYPE_TINY => ShortEncoderDecoder
