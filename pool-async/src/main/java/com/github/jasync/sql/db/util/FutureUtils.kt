...
fun <A> CompletableFuture<A>.failed(e: Throwable): CompletableFuture<A> = this.also {
...
inline fun <A> CompletableFuture<A>.onComplete(crossinline onCompleteFun: (Try<A>) -> Unit): CompletableFuture<A> =
    whenComplete { a, t ->
        onCompleteFun(if (t != null) Try.raise(t) else Try.just(a))
    }
...
inline fun <A> CompletableFuture<A>.onCompleteAsync(
    ...
    whenCompleteAsync(BiConsumer { a, t ->
        onCompleteFun(if (t != null) Try.raise(t) else Try.just(a))
    }, executor)
